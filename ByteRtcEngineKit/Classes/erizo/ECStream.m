...
//@interface JExternalVideoSource : RTCVideoSource
//- (instancetype)init;
...
@interface ECStream ()
@property (strong, nonatomic) RTCVideoSource *externalSource;
@property (strong, nonatomic) RTCVideoCapturer *externalCapturer;
@end
...
- (void)generateVideoTracks {
    ...
    if (localVideoTrack) {
        [_mediaStream addVideoTrack:localVideoTrack];  // TODO: zhangle
    } else {
...
- (RTCVideoTrack *)createLocalVideoTrack {
    ...
    if ([(NSNumber *)[_streamOptions objectForKey:kStreamOptionExternalVideoSource] boolValue]) {
        _externalSource = [_peerFactory videoSource];
        _externalCapturer = [[RTCVideoCapturer alloc] initWithDelegate:_externalSource];
        localVideoTrack = [_peerFactory videoTrackWithSource:_externalSource trackId:kLicodeVideoLabel];
    }
    else {
...
- (void)videoView:(RTCEAGLVideoView *)videoView didChangeVideoSize:(CGSize)size {
    if(self.delegate != nil) {
        [self.delegate didChangeVideoSize:size streamId:self.streamId streamAttributes:self.streamAttributes];
    }
}

- (void)pushExternalVideoFrame:(RTCVideoFrame *)frame {
    if (_externalSource && _externalCapturer) {
        [_externalSource capturer:_externalCapturer didCaptureVideoFrame:frame];
    }
}
